...
declare const UIC: any; // Use declare if you import via CDN. Regular Angular (node_modules) usage would be via an import
...
export class DdsComponent implements OnInit, AfterViewInit {
  ...
  initializeNow = () => {
    ...
    if (this.ddsElement) {
      if (UIC[ddsCom]) {
        this.ddsComponent = new UIC[ddsCom](this.ddsElement, this.ddsOptions);
      } else {
        console.error(`No such UIC Component, ${ddsCom}`);
      }
    } else {
      console.error(
        `UIC Component "${ddsCom}" not found by elementId "${this.elementId}".`
      );
    }
  };
    ...
    const observerDefs: Array<ObserverDef> = [
      {
        ...
        callback: (elem: any): void => {
          this.ddsComponent = new UIC[ddsCom](elem, this.ddsOptions);
        }
      }
    ];
